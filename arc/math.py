"""
Migrated from 
- https://github.com/deepseek-ai/DeepGEMM/blob/main/deep_gemm/utils/math.py
...
def symm_square_rel_L2_error(x: torch.Tensor, y: torch.Tensor):
    """
    Symmetric square relative L2 error, defined as 1 - 2 * <x, y> / (<x, x> + <y, y>)
    """
...
def per_token_cast_to_fp8(x: torch.Tensor, use_ue8m0: bool, gran_k: int = 128, use_packed_ue8m0: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
    assert x.dim() == 2
    m, n = x.shape
...
def per_token_cast_to_fp4(x: torch.Tensor, use_ue8m0: bool, gran_k: int = 128, use_packed_ue8m0: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
    m, n = x.shape
    assert n % 2 == 0
...
def cast_back_from_fp4(packed: torch.Tensor, sf: torch.Tensor, gran_k: int = 128, use_packed_ue8m0: bool = False) -> torch.Tensor:
    m, n2 = packed.shape
    n = n2 * 2
    if use_packed_ue8m0:
