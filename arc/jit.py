"""
Adapted from https://github.com/sgl-project/sglang/blob/main/python/sglang/jit_kernel/utils.py
"""
...
def _resolve_kernel_path() -> pathlib.Path:
    ...
    def _environment_install():
        ...
        if (candidate / "include").exists() or (candidate / "csrc").exists():
            return candidate
        return None
    ...
    if path is None:
        raise RuntimeError("Cannot find arc JIT kernel path")
    return path
...
def _resolve_impl_path(
    impl_file: str | pathlib.Path,
    include_paths: List[str] | None = None,
) -> pathlib.Path:
    path = pathlib.Path(impl_file)
    if path.is_absolute():
...
def _impl_label(path: pathlib.Path) -> str:
    try:
        return str(path.relative_to(KERNEL_PATH))
    except ValueError:
        return str(path)
...
def get_cutlass_include_paths() -> List[str]:
    ...
    cutlass_dir = pathlib.Path(__file__).resolve().parent.parent / "thirdparty" / "cutlass"
    candidates = [
        cutlass_dir / "include",
    ...
    if not unique_paths:
        raise RuntimeError("Cannot find CUTLASS headers required for JIT compilation. Please install flashinfer or deep_gemm with CUTLASS headers.")
    return unique_paths
